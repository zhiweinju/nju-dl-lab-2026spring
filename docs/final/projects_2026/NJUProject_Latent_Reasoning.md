...
本项目不强制要求复现 CODI 或 CoLaR，但鼓励将其中的思想作为改进方向参考。

### 3.4 鼓励的改进方向
在完成基础 Coconut-style latent reasoning 后，鼓励同学们尝试以下任一方向：

- 改进 latent state transition 方式；
- 引入 auxiliary loss；
- 研究不同 latent step 数量的设置；
- 尝试简单的动态 latent length 或压缩策略；
- 研究不同训练目标对 latent reasoning 的影响。
...
4. **实验结果**
   ...
   - latent step 分析
5. **结果讨论**
   - latent reasoning 的优缺点
   - 与显式 CoT 的差异
   - latent length 与准确率之间的关系
6. **结论**
7. **参考文献**
...
- 报告应条理清晰、表达准确；
- 图表需有标题与必要说明；
- 若参考了其他论文、代码库或开源实现，需要在报告中注明；
- 若由于硬件资源限制对实验设置进行了简化，也需要在报告中说明。
